...
export default function MyGames() {
	...
	return (
		<ScrollView style={styles.container}>
			<Input style={{ marginBottom: 20 }} placeholder='Введите название игры' value={searchQuery} onChangeText={(e) => setSearchQuery(e)}/>
			<Button buttonText="Искать" onPress={submit}/>
			<View style={styles.cardList}>
				{games.length == 0 && <Text style={styles.noGames}>Вы пока ничего не искали</Text>}
				{games.results?.map((g: any, i: number) => (
					<GameItem name={g.name} key={i} image={g.background_image} rate={g.rating} released={g.released}/>
				))}
			</View>
		</ScrollView>
	);
